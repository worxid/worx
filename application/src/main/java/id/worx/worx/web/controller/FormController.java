...
public class FormController implements SecuredRestController {
    ...
    public ResponseEntity<Page<SearchFormDTO>> search(
        ...
        List<SearchFormDTO> dtos = forms.stream()
                ...
                .collect(Collectors.toList());
        Page<SearchFormDTO> page =
                new BasePageResponse<>(dtos, forms.getPageable(), forms.getTotalElements());
        return ResponseEntity.status(HttpStatus.OK)
                .body(page);
    }
    ...
    public ResponseEntity<ByteArrayResource> export(@RequestBody @Valid FormExportRequest request) {
        String filename = "";
        String contentType = "";
        ByteArrayOutputStream reportByte;
        String option = request.getOption().toUpperCase();

        if (option.equals("DOCX")) {
            reportByte =
                    formExportService.saveFormAsDOCX(request.getFormId());
            filename = "forms.docx";
            contentType = "application/octet-stream";
        } else if (option.equals("PDF")) {
            reportByte =
                    formExportService.saveFormAsPDF(request.getFormId());
            filename = "forms.pdf";
